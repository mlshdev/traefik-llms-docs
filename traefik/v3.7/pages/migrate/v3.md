...
section: "Migrate"
breadcrumb: "Migrate / Traefik v3 minor migrations"
traefik_version: "v3.7"
upstream_path: "docs/content/migrate/v3.md"
source_url: "https://github.com/traefik/traefik/blob/21a4ca1fad46ceca9b7d7903eeaf3721325f3e50/docs/content/migrate/v3.md"
---
...
Therefore, in the corresponding RBACs (see [KubernetesIngressNGINX](https://raw.githubusercontent.com/traefik/traefik/21a4ca1fad46ceca9b7d7903eeaf3721325f3e50/docs/content/reference/dynamic-configuration/kubernetes-ingress-nginx-rbac.yml) provider RBACs) the `configmaps` right has been added.
...
Starting with `v3.6.26`, the new `core.strictTLSOptions` install configuration option disables this fallback:
the routers involved in the conflict are marked in error and are not built at all.
...
Starting with `v3.6.26`, the Kubernetes CRD provider derives the names it generates from the identity of the
object they come from, instead of joining its namespace and name, which could produce the same name for two
distinct objects, one silently replacing the other.
...
Starting with `v3.6.26`, the new `defaultTLSResourcesNamespace` provider option reserves these resources
to a namespace the cluster operator controls.
